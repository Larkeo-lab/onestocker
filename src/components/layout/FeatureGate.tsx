...
const PATHS: Record<CreditFeature, string> = {
  generate: APP_PATH,
 ...
 * ระหว่างโหลดหรือโหลดไม่สำเร็จแสดงหน้าตามปกติ (ดู useFeatureEnabled)
 */
export function FeatureGate({ feature, children }: { feature: CreditFeature; children: ReactNode }) {
  const { t } = useTranslation()
  const enabled = useCreditCosts().data?.enabled

  if (!enabled || enabled[feature]) return children

  const names: Record<CreditFeature, string> = {
    generate: t('nav.generate'),
    removeBg: t('nav.remove-bg'),
  }
  // พาไปงานอื่นที่ยังเปิดอยู่ ถ้าปิดหมดก็ไม่มีปุ่มให้ไปต่อ
  const other: CreditFeature = feature === 'generate' ? 'removeBg' : 'generate'
